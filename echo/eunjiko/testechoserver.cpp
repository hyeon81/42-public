...
int main() {
    ...
    //kevent 구조를 초기화
    EV_SET(&events[0], serverSocket, EVFILT_READ, EV_ADD, 0, 0, NULL); //struct kevent 배열인 events를 사용하여 EV_SET 함수를 호출하여 서버 소켓의 읽기 이벤트를 설정하고 활성화
    ...
    */
    ...
    while (true) { 
        //kqueue에 이벤트 첨부
        int nev = kevent(kq, events, 1, NULL, 0, NULL); // kevent 함수를 사용하여 새 이벤트를 기다리거나 (클라이언트 연결 시도) 이전에 등록한 이벤트를 모니터링합니다.
        //kq로 전달된 kqueue에 새로 모니터링할 이벤트를 등록하고, 발생하여 아직 처리되지 않은(pending 상태인) 이벤트의 개수를 return
        if (nev == -1) {
            perror("kevent");
            return -1;
        }
        if (events[0].flags & EV_EOF) { //이벤트의 flags 필드를 검사하여 연결이 끊어졌는지 확인 EV_EOF 플래그를 사용하여 클라이언트 연결이 종료되었는지 확인
            close(serverSocket); // 연결이 종료된 경우, 클라이언트 소켓을 닫고 해당 클라이언트에 대한 모니터링을 중지
            return 0;
        } 
        else if (events[0].ident == serverSocket) { //ident 필드를 사용하여 이벤트가 발생한 소켓을 확인
        ...
        else { //이벤트가 serverSocket이나 연결 종료 이벤트가 아닌 경우에는 데이터를 읽고 에코하는 클라이언트와의 통신이 이루어진다
